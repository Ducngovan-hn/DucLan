...
**⚠️ DMO TRƯỚC, THIẾU MỚI LẤY NGOÀI (chỉ thị anh Đức 15/09):** khi cần số liệu:
1. **Có trong DMO → DÙNG DMO** (`production/dmo/DMO-*.md`): tiền thực nhận, tiền trả hàng/khách
   mỗi ngày, %, việc đã làm… Đọc file text local vừa nhanh vừa rẻ token; anh Đức tổng hợp DMO để dùng lại.
2. **KHÔNG có trong DMO → LẤY NGUỒN NGOÀI** (Zalo qua `zalo_dump.mjs`, Excel 6 lọ…). Vd giai đoạn
   trước 02/09 chưa có DMO → lấy Zalo cho phần đó; ngày chưa chốt → lấy nguồn thật.
Ví dụ biểu đồ tiền trả hàng → `tools/bieu_do_tra_hang.py` đọc từ DMO (bỏ số < 100.000 để không cộng
nhầm đơn giá 6.500/8.500); dải nào thiếu DMO thì bổ sung từ Zalo.
